...
/**
 * modified by Arnulf Becker, BTU Cottbus-Senftenberg, 2015
 * See the bachelor thesis: Festkommaportierung des generalisierten Mel-Cepstrum
 * https://www.tu-cottbus.de/kommunikationstechnik/wiki/index.php/BA_Arnulf_Becker
 */
...
INT16 dlm_mgcepfix(INT16* input, INT32 n, INT16* output, INT16 order,
	...
	for (i = 0; i < n; i++)
		in[i] = (FLOAT64) input[i] / 32767. * SIG_NRM;
	ret = dlm_mgcep(in, n, out, order, (FLOAT64) gamma / 32767.,
			(FLOAT64) lambda / 32767., 32768.);
	for (i = 0; i < order; i++)
		output[i] = (INT16) round(out[i] * 32767. / RES_NRM);
