 ...
 *
 * AUTHOR  : Kaz Kylheku
 * UPDATE  : $Date: 2011-03-03 23:14:48 +0100 (Do, 03 Mrz 2011) $, $Author: strecha $
 *           $Revision: 1644 $
 * PACKAGE : dlabpro/base
 * RCS-ID  : $Id: hash.c 1644 2011-03-03 22:14:48Z strecha $
 */
 ...
 *
 * $Id: hash.c 1644 2011-03-03 22:14:48Z strecha $
 * $Name$
 */

/* MW 2004-07-30 HACK: Disable MSVC warnings --> */
#ifdef _MSC_VER
  ...
  #pragma warning( disable : 4706 ) /* Zuweisung in bedingtem Ausdruck */
#endif
/* <-- */
/* MW 01-04-04 TODO: Uncomment the following line to engage the hash_verify function --> */
/* #define __KL_HASHVERIFY */
/* <-- */
...
#ifdef KAZLIB_RCSID
static const char rcsid[] = "$Id: hash.c 1644 2011-03-03 22:14:48Z strecha $";
#endif
...
static hash_val_t hash_fun_default(const void *key, void *context);
static int hash_comp_default(const void *key1, const void *key2, void *context);
 ...
 * 9. 2004-02-24 MWX: User defined pointer to be passed to user supplied hash
 *    and comparison functions; may be NULL
 */

hash_t *hash_create(hashcount_t maxcount, hash_comp_t compfun,
  hash_fun_t hashfun, void* context)
{
    hash_t *hash;
  ...
  if (hash->table) {  /* 5 */
      ...
      /*hash->context = NULL;  9 MWX */
      hash->mask = INIT_MASK;
      hash->dynamic = 1;      /* 7 */
      hash->context = context;  /* 9 MWX */
      clear_table(hash);      /* 8 */
      assert (hash_verify(hash));
      return hash;
  }
 ...
 * 5. 2004-02-24 MWX: Pass user defined context pointer to hash function
 */
...
void hash_insert(hash_t *hash, hnode_t *node, const void *key)
{
  ...
  grow_table(hash);
    ...
    hkey = hash->function(key,hash->context /* 5 */);
 ...
 *    entry.
 * 4. 2004-02-24 MWX: Pass user defined context pointer to hash and comparison
 *    functions
 */
...
hnode_t *hash_lookup(hash_t *hash, const void *key)
{
    ...
    hkey = hash->function(key,hash->context /* 4 */);    /* 1 */
    chain = hkey & hash->mask;    /* 2 */

    for (nptr = hash->table[chain]; nptr; nptr = nptr->next) {  /* 3 */
  if (nptr->hkey == hkey && hash->compare(nptr->key,key,hash->context /* 4 */) == 0)
      return nptr;
    }
